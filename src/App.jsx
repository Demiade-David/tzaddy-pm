...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
// Set this to your Render.com backend URL, e.g. "https://tzaddy-pm.onrender.com"
// Leave empty string to use relative paths (if frontend is served from same server)
...
const api = {
  ...
  patch:  (path, body) => fetch(`${API_BASE}${path}`, { method:"PATCH",  headers:{"Content-Type":"application/json"}, body:JSON.stringify(body) }).then(r => r.json()),
...
const ls = {
  ...
  set: (k, v) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch(e) { console.error("localStorage write failed:", e); } },
};
...
function readFileAsBase64(file) {
  return new Promise((resolve, reject) => {
    if (file.size > MAX_PDF_BYTES) {
      reject(new Error(`File too large. Max 3.5 MB. This file is ${(file.size/1024/1024).toFixed(1)} MB.`));
      return;
    }
...
// Upload PDF to backend. refType = "engagement" | "payment"
async function uploadPdf(refType, refId, clientId, file) {
  ...
  const result = await api.post("/api/attachments", payload);
  if (result.error) throw new Error(result.error);
  return result;
}

async function deletePdf(refType, refId) {
  return api.del(`/api/attachments/${refType}/${refId}`);
}

async function fetchAttachmentsForClient(clientId) {
  return api.get(`/api/attachments?clientId=${clientId}`);
}

// ─── PDF UI COMPONENTS ────────────────────────────────────────────────────────
function PdfAttachment({ doc, onRemove, compact = false }) {
  if (!doc) return null;
  const open = () => {
    const w = window.open("","_blank");
    if (!w) { alert("Allow pop-ups to view PDF."); return; }
    w.document.write(`<!DOCTYPE html><html><head><meta charset=utf-8><title>${doc.name}</title><style>body{margin:0;background:#222}iframe{width:100vw;height:100vh;border:none}</style></head><body><iframe src="${doc.data}"></iframe></body></html>`);
    w.document.close();
  };
  if (compact) return (
    <div style={{display:"flex",alignItems:"center",gap:"6px",background:"#f0ede5",borderRadius:"2px",padding:"4px 8px",fontSize:"10px"}}>
      <span style={{color:"#555"}}>📄</span>
      <span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"#555",maxWidth:"140px"}}>{doc.name}</span>
      <button onClick={open} style={{background:"none",border:"none",cursor:"pointer",color:"#1a6ba0",fontSize:"10px",letterSpacing:"0.5px",fontFamily:"'IBM Plex Mono',monospace",padding:0}}>View</button>
      {onRemove && <button onClick={onRemove} style={{background:"none",border:"none",cursor:"pointer",color:"#b52a2a",fontSize:"12px",lineHeight:1,padding:0}}>×</button>}
    </div>
  );
  return (
    <div style={{display:"flex",alignItems:"center",gap:"10px",background:"#f0ede5",borderRadius:"2px",padding:"10px 14px",marginTop:"8px"}}>
      <span style={{fontSize:"20px"}}>📄</span>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:"12px",fontWeight:"600",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{doc.name}</div>
        <div style={{fontSize:"10px",color:"#888",marginTop:"2px"}}>{doc.size} · Attached {dt(doc.attachedAt)}</div>
      </div>
      <div style={{display:"flex",gap:"6px",flexShrink:0}}>
        <Btn size="sm" variant="outline" onClick={open}>View</Btn>
        {onRemove && <Btn size="sm" variant="danger" onClick={onRemove}>Remove</Btn>}
      </div>
    </div>
  );
}

function PdfUploader({ refType, refId, clientId, onUploaded, onRemove, existing = null, label = "Attach PDF" }) {
  ...
  const handle = async (e) => {
    ...
    try {
      const saved = await uploadPdf(refType, refId, clientId, file);
      onUploaded(saved);
    } catch (ex) {
      setErr(ex.message);
    }
  ...
  if (existing) return (
    <div>
      <PdfAttachment doc={existing} onRemove={async () => { await deletePdf(refType, refId); onRemove(); }} />
    </div>
  );

  // Can only upload once refId exists
  if (!refId) return (
    <div style={{fontSize:"10px",color:"#aaa",fontStyle:"italic"}}>Save the record first to attach a PDF.</div>
  );

  return (
    <div>
      <input ref={ref} type="file" accept="application/pdf" onChange={handle} style={{display:"none"}} />
      <button
        onClick={() => ref.current?.click()}
        disabled={loading}
        style={{background:"none",border:"1px dashed #bbb",padding:"8px 14px",fontSize:"10px",letterSpacing:"1px",textTransform:"uppercase",cursor:"pointer",borderRadius:"2px",fontFamily:"'IBM Plex Mono',monospace",color:"#777",display:"flex",alignItems:"center",gap:"6px"}}
      >
        <span>📎</span> {loading ? "Uploading…" : label}
      </button>
      {err && <div style={{fontSize:"10px",color:"#b52a2a",marginTop:"5px"}}>{err}</div>}
    </div>
  );
}
...
function PageTitle({children, actions}) {
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"22px",paddingBottom:"10px",borderBottom:"2px solid #0a0a0a"}}>
...
export default function TzaddyPM() {
  ...
  useEffect(() => {
    setClients(ls.get("tz_c", []));
    setEngagements(ls.get("tz_e", []));
    setPayments(ls.get("tz_p", []));
    setReady(true);
  }, []);

  // Persist to localStorage on every change
  useEffect(() => { if (ready) ls.set("tz_c", clients);     }, [clients,     ready]);
  useEffect(() => { if (ready) ls.set("tz_e", engagements); }, [engagements, ready]);
  useEffect(() => { if (ready) ls.set("tz_p", payments);    }, [payments,    ready]);
  ...
  const updPay    = (id,patch) => setPayments(p => p.map(pay => pay.id===id ? {...pay,...patch} : pay));

  if (!ready) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontFamily:"monospace",fontSize:"12px",color:"#888",background:"#f5f4f0"}}>Loading…</div>
  );
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1.5fr",gap:"14px"}}>
        ...
        <div style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",padding:"20px"}}>
          ...
          {action.length===0
            ? <div style={{color:"#bbb",fontSize:"12px",textAlign:"center",padding:"20px 0"}}>All clear.</div>
...
// ─── CLIENTS ─────────────────────────────────────────────────────────────────
function Clients({clients,engagements,payments,addClient,delClient}) {
  ...
  const save = () => {
    ...
    addClient({...f});
    setF({name:"",business:"",phone:"",email:""});
    setShow(false);
  };

  // Load all PDFs for this client from backend
  useEffect(() => {
    if (!detail) { setDocs([]); return; }
    fetchAttachmentsForClient(detail).then(setDocs).catch(() => setDocs([]));
  }, [detail]);

  const detailClient = clients.find(c => c.id===detail);
  const clientEngs   = detail ? engagements.filter(e => e.clientId===detail) : [];
  const clientPays   = detail ? payments.filter(p => p.clientId===detail)    : [];

  if (detail && detailClient) return (
    <div>
      <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"22px"}}>
        <button onClick={()=>setDetail(null)} style={{background:"none",border:"none",cursor:"pointer",color:"#888",fontFamily:"'IBM Plex Mono',monospace",fontSize:"11px",letterSpacing:"1px"}}>← Clients</button>
        <span style={{color:"#ddd"}}>/</span>
        <span style={{fontFamily:"'Playfair Display',serif",fontSize:"17px",fontWeight:"700"}}>{detailClient.name}</span>
        {detailClient.business && <span style={{color:"#888",fontSize:"13px"}}>— {detailClient.business}</span>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px",marginBottom:"24px"}}>
        <div style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",padding:"20px"}}>
          <div style={{fontSize:"10px",letterSpacing:"1.5px",textTransform:"uppercase",color:"#666",marginBottom:"12px",fontWeight:"600"}}>Contact</div>
          {[["Phone",detailClient.phone],["Email",detailClient.email],["Added",dt(detailClient.createdAt)]].map(([l,v]) => (
            <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #f5f4f0",fontSize:"12px"}}>
              <span style={{color:"#888"}}>{l}</span><span style={{fontWeight:"600"}}>{v||"—"}</span>
            </div>
          ))}
        </div>
        <div style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",padding:"20px"}}>
          <div style={{fontSize:"10px",letterSpacing:"1.5px",textTransform:"uppercase",color:"#666",marginBottom:"12px",fontWeight:"600"}}>Summary</div>
          {[["Engagements",clientEngs.length],["Invoiced",N(sum(clientEngs.map(e=>e.totalFee)))],["Received",N(sum(clientPays.map(p=>p.amount)))],["Documents",docs.length]].map(([l,v]) => (
            <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #f5f4f0",fontSize:"12px"}}>
              <span style={{color:"#888"}}>{l}</span><span style={{fontWeight:"600"}}>{v}</span>
            </div>
          ))}
      ...
      {docs.length > 0 && (
        <div style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",padding:"20px",marginBottom:"16px"}}>
          <div style={{fontSize:"10px",letterSpacing:"1.5px",textTransform:"uppercase",color:"#666",marginBottom:"14px",fontWeight:"600"}}>Attached Documents ({docs.length})</div>
          {docs.map((doc,i) => (
            <div key={i} style={{marginBottom:"10px"}}>
              <div style={{fontSize:"10px",color:"#999",marginBottom:"4px",letterSpacing:"0.5px"}}>
                {doc.refType === "engagement" ? "Engagement" : "Payment"} · {dt(doc.attachedAt)}
              </div>
              <PdfAttachment doc={doc} compact={false} />
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",padding:"20px"}}>
        <div style={{fontSize:"10px",letterSpacing:"1.5px",textTransform:"uppercase",color:"#666",marginBottom:"14px",fontWeight:"600"}}>Engagements</div>
        {clientEngs.length===0
          ? <div style={{color:"#bbb",fontSize:"12px",textAlign:"center",padding:"16px 0"}}>No engagements yet.</div>
          : clientEngs.map(e => {
              const doc = docs.find(d => d.refType==="engagement" && d.refId===e.id);
              return (
                <div key={e.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:"1px solid #f5f4f0",fontSize:"12px"}}>
                  <div>
                    <div style={{display:"flex",gap:"5px",flexWrap:"wrap",marginBottom:"3px"}}>{(e.services||[]).map(s=><span key={s} style={{background:"#f0ede5",padding:"2px 7px",borderRadius:"2px",fontSize:"10px",color:"#666"}}>{s}</span>)}</div>
                    <div style={{color:"#888",fontSize:"11px"}}>{dt(e.createdAt)} · {N(e.totalFee)}</div>
                  </div>
                  <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                    {doc && <PdfAttachment doc={doc} compact={true}/>}
                    <Badge stage={e.stage}/>
                  </div>
                </div>
              );
  ...
  return (
    <div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
            ...
            <tbody>{clients.map(c => (
              <tr key={c.id} className="rh">
                <td style={{padding:"10px 14px",borderBottom:"1px solid #f0ede5",fontWeight:"600"}}>
                  <button onClick={()=>setDetail(c.id)} style={{background:"none",border:"none",cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",fontWeight:"600",fontSize:"12px",color:"#0a0a0a",textDecoration:"underline",padding:0}}>{c.name}</button>
                </td>
      ...
      {show && (
        <Modal title="Add Client" onClose={()=>setShow(false)}>
...
function Pipeline({clients,engagements,payments,addClient,addEng,updEng,delEng}) {
  ...
  const [engDocs, setEngDocs] = useState({}); // { engId: attachmentDoc }

  const fil = sf==="All" ? engagements : engagements.filter(e => e.stage===sf);

  // Load PDF for an engagement when it expands
  const loadEngDoc = async (engId, clientId) => {
    if (engDocs[engId] !== undefined) return; // already fetched
    try {
      const doc = await api.get(`/api/attachments/engagement/${engId}`);
      setEngDocs(p => ({...p, [engId]: doc.error ? null : doc}));
    } catch {
      setEngDocs(p => ({...p, [engId]: null}));
    }
  };
  ...
  const handleExpand = (e) => {
    ...
    if (exp !== id) loadEngDoc(id, e.clientId);
  };
  ...
  return (
    <div>
      ...
      {fil.length===0 ? <Empty msg="No engagements in this stage."/> : fil.map(e => {
        ...
        const doc = engDocs[e.id] || null;

        return (
          <div key={e.id} style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",marginBottom:"8px"}}>
            <div onClick={()=>handleExpand(e)} style={{padding:"14px 18px",display:"flex",alignItems:"flex-start",justifyContent:"space-between",cursor:"pointer"}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",gap:"18px",fontSize:"11px",color:"#888"}}>
                  ...
                  {doc && <span style={{color:"#555"}}>📄 {doc.name}</span>}
                </div>
              </div>
            ...
            {isO && (
              <div style={{borderTop:"1px solid #f0ede5",padding:"16px 18px",background:"#fafaf7"}}>
                ...
                <div style={{marginBottom:"14px"}}>
                  <div style={{fontSize:"9px",letterSpacing:"1.5px",textTransform:"uppercase",color:"#999",marginBottom:"8px"}}>Document</div>
                  <PdfUploader
                    refType="engagement"
                    refId={e.id}
                    clientId={e.clientId}
                    existing={doc}
                    onUploaded={saved => setEngDocs(p => ({...p,[e.id]:saved}))}
                    onRemove={() => setEngDocs(p => ({...p,[e.id]:null}))}
                    label="Attach PDF to this engagement"
                  />
                </div>
...
function LogModal({clients,addClient,addEng,onClose}) {
  ...
  // PDF is attached after engagement is created (needs an ID first)
  const [createdEngId,   setCreatedEngId]   = useState(null);
  ...
  const create = () => {
    ...
    const engId = uid();
    addEng({id:engId,clientId:cid,services,feeItems,totalFee:tot,myFee:Number(myFee)||0,notes,stage:"In Discussion",tasks:services.flatMap(s=>(SERVICE_TASKS[s]||[]).map(t=>({id:uid(),text:t,done:false})))});
    setCreatedEngId(engId);
    setCreatedClientId(cid);
  };

  // After engagement is created, show PDF attach step
  if (createdEngId) return (
    <Modal title="Engagement Created" onClose={onClose}>
      <div style={{background:"#edfaee",border:"1px solid #81c784",padding:"10px 14px",borderRadius:"2px",fontSize:"11px",marginBottom:"18px",color:"#2e7d32",fontWeight:"600"}}>✓ Engagement logged successfully.</div>
      <div style={{fontSize:"12px",color:"#555",marginBottom:"14px"}}>Attach a supporting document to this engagement (optional).</div>
      <PdfUploader
        refType="engagement"
        refId={createdEngId}
  ...
  return (
    <Modal title="Log from Conversation" onClose={onClose} width={680}>
      ...
      {!parsed ? (
        ...
        <div>
          ...
          <div style={{marginBottom:"16px"}}>
            ...
            {cMode==="existing"
              ? <TS value={selId} onChange={setSelId}><option value="">— Choose —</option>{clients.map(c=><option key={c.id} value={c.id}>{c.name}{c.business?` — ${c.business}`:""}</option>)}</TS>
              : (
          ...
          <div style={{marginBottom:"16px"}}>
            <div style={{fontSize:"9px",letterSpacing:"1.5px",textTransform:"uppercase",color:"#666",marginBottom:"10px",fontWeight:"600"}}>Services</div>
...
function GmailModal({eng, cl, type, pays, onClose}) {
  ...
  return (
    <Modal title={type==="invoice"?"Send Invoice by Email":"Send Proposal by Email"} onClose={onClose} width={640}>
...
function NewEngModal({clients,addClient,addEng,onClose}) {
  ...
  const create = () => {
    ...
    if (!cid) return alert("Select a client.");
    const engId = uid();
    addEng({id:engId,clientId:cid,services:svcs,feeItems:fi,totalFee:tot,myFee:Number(mf)||0,notes,stage:"In Discussion",tasks:svcs.flatMap(s=>(SERVICE_TASKS[s]||[]).map(t=>({id:uid(),text:t,done:false})))});
    setCreatedEngId(engId);
    setCreatedClientId(cid);
  };

  // After creation — optional PDF attach
  if (createdEngId) return (
    <Modal title="Engagement Created" onClose={onClose}>
      <div style={{background:"#edfaee",border:"1px solid #81c784",padding:"10px 14px",borderRadius:"2px",fontSize:"11px",marginBottom:"18px",color:"#2e7d32",fontWeight:"600"}}>✓ Engagement created.</div>
      <div style={{fontSize:"12px",color:"#555",marginBottom:"14px"}}>Attach a document (optional).</div>
      <PdfUploader
        refType="engagement"
        refId={createdEngId}
        clientId={createdClientId}
  ...
  return (
    <Modal title={`New Engagement — Step ${step} of 4`} onClose={onClose} width={600}>
      ...
      {step===1 && (
        <div>
          ...
          {cm==="existing"
            ? <Field label="Client"><TS value={sid} onChange={setSid}><option value="">— Choose —</option>{clients.map(c=><option key={c.id} value={c.id}>{c.name}{c.business?` — ${c.business}`:""}</option>)}</TS></Field>
            : (
      ...
      {step===3 && (
        <div>
          ...
          <Field label="My Professional Fee (N)">
            <input type="number" value={mf} onChange={e=>setMf(e.target.value)} placeholder="Your portion" style={inp}/>
          </Field>
...
// ─── PAYMENTS ────────────────────────────────────────────────────────────────
function Payments({clients,engagements,payments,addPay,updPay}) {
  const [mod,        setMod]        = useState(null);
  const [em,         setEm]         = useState(null);
  const [f,          setF]          = useState({amount:"",myPortion:"",date:"",notes:""});
  const [payDocs,    setPayDocs]    = useState({}); // { payId: doc }
  const [createdPayId,    setCreatedPayId]    = useState(null);
  const [createdClientId, setCreatedClientId] = useState(null);
  ...
  // Load PDF for a payment record
  const loadPayDoc = async (payId) => {
    if (payDocs[payId] !== undefined) return;
    try {
      const doc = await api.get(`/api/attachments/payment/${payId}`);
      setPayDocs(p => ({...p,[payId]: doc.error ? null : doc}));
    } catch {
      setPayDocs(p => ({...p,[payId]: null}));
    }
  };
...
useEffect(() => {
  ...
  const rec = () => {
    ...
    const payId = uid();
    addPay({id:payId,engagementId:mod.id,clientId:mod.clientId,amount:Number(f.amount),myPortion:Number(f.myPortion)||0,date:f.date,notes:f.notes});
    setCreatedPayId(payId);
    setCreatedClientId(mod.clientId);
    setMod(null);
  };

  // After payment recorded — optional PDF attach
  if (createdPayId) return (
    <div>
      <PageTitle>Payments</PageTitle>
      <Modal title="Payment Recorded" onClose={() => { setCreatedPayId(null); setCreatedClientId(null); }}>
        <div style={{background:"#edfaee",border:"1px solid #81c784",padding:"10px 14px",borderRadius:"2px",fontSize:"11px",marginBottom:"18px",color:"#2e7d32",fontWeight:"600"}}>✓ Payment recorded.</div>
        <div style={{fontSize:"12px",color:"#555",marginBottom:"14px"}}>Attach a receipt or bank credit alert (optional).</div>
        <PdfUploader
          refType="payment"
          refId={createdPayId}
          clientId={createdClientId}
          onUploaded={(doc) => { setPayDocs(p => ({...p,[createdPayId]:doc})); setCreatedPayId(null); setCreatedClientId(null); }}
          onRemove={() => {}}
          label="Attach Receipt PDF"
        />
        <div style={{marginTop:"16px"}}>
          <Btn variant="ghost" onClick={() => { setCreatedPayId(null); setCreatedClientId(null); }}>Skip — Close</Btn>
        </div>
      </Modal>
    </div>
  );
  ...
  return (
    <div>
      ...
      {payable.length===0 ? <Empty msg="No active engagements."/> : payable.map(e => {
        ...
        return (
          <div key={e.id} style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",marginBottom:"8px",padding:"16px 18px"}}>
            ...
            {ep.length>0 && (
              <div style={{marginTop:"10px",background:"#fafaf7",borderRadius:"2px",padding:"10px 12px"}}>
                {ep.map(p => {
                  const doc = payDocs[p.id];
                  return (
                    <div key={p.id} style={{padding:"6px 0",borderBottom:"1px solid #eee"}}>
                      <div style={{display:"flex",gap:"20px",fontSize:"11px",marginBottom:doc?"6px":"0"}}>
                      ...
                      {doc && (
                        <PdfAttachment doc={doc} compact={true}
                          onRemove={async () => { await deletePdf("payment", p.id); setPayDocs(prev => ({...prev,[p.id]:null})); }}
                        />
                      )}
                    </div>
                  );
      ...
      {payments.length>0 && (
        <div style={{marginTop:"28px"}}>
          ...
          <div style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",overflow:"hidden"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
              <thead><tr style={{background:"#fafaf7"}}>{["Date","Client","Received","Warehoused","Disbursements","Doc","Notes"].map(h => (
                <th key={h} style={{textAlign:"left",padding:"10px 14px",fontSize:"9px",letterSpacing:"1.5px",textTransform:"uppercase",color:"#999",borderBottom:"1px solid #e0ddd5",fontWeight:"600"}}>{h}</th>
              ))}</tr></thead>
              <tbody>{[...payments].reverse().map(p => {
                ...
                return (
                  <tr key={p.id} className="rh">
                    ...
                    <td style={{padding:"10px 14px",borderBottom:"1px solid #f0ede5"}}>
                      {doc
                        ? <PdfAttachment doc={doc} compact={true} onRemove={async()=>{ await deletePdf("payment",p.id); setPayDocs(prev=>({...prev,[p.id]:null})); }}/>
      ...
      {mod && (
        <Modal title="Record Payment" onClose={()=>setMod(null)}>
  ...
  return (
    <div>
      ...
      {active.length===0 ? <Empty msg="No active jobs."/> : active.map(e => {
        ...
        return (
          <div key={e.id} style={{background:"#fff",border:"1px solid #e0ddd5",borderRadius:"3px",marginBottom:"14px",padding:"20px"}}>
            ...
            {all
              ? (<div style={{background:"#edfaee",border:"1px solid #81c784",padding:"10px 14px",borderRadius:"2px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
